...
/** Subscribe: retorna unsubscribe — dados prontos para Kanban (agrupados por stage) */
...
export async function addLeadsBulkWithCampaign(shopId, rows, { campaign, assignToSellerUid = null }) {
    ...
    for (let i = 0; i < rows.length; i += chunk) {
        ...
        rows.slice(i, i + chunk).forEach((r) => {
            const lead = {
                ...
                stage: "Novo",
                sellerUid: assignToSellerUid || null, // <- atribuição em massa (ou não)
                createdAt: new Date(),
                updatedAt: new Date(),
            };
